...
def run(logID, conf):
    ...
    for i in range(conf.initialTimeStep, conf.initialTimeStep + conf.simHorizon):
        ...
        if conf.is3D:
            ...
            z_t_ground_3D, z_t_objects_3D = z_t_3D.BEV_GroundSeg(grid_res=conf.resolution)
            z_t_ground = z_t_ground_3D.convertTo2D() if conf.freeUpGroundDetections else None
